...
<AccordionGroup>
  ...
  <Accordion title="Multiple default communities">
    `GetDefaultCommunity` has no `ORDER BY`. If more than one non-test community has `is_default`, the fallback is whichever row Postgres returns first. `MoveSchoolToDefaultCommunity` has the same issue.
  </Accordion>
  <Accordion title="Archived current community with no local match">
    `communityAtLocation` calls `GetSearchBounds` for the user's current community, and that query filters out archived rows. A user still assigned to an archived community, standing outside every eligible box, gets a server error from resolve instead of a move to the default.
  </Accordion>
