...
| Ride stops with no address, or a relative label such as "Current location" | `ReverseGeocodeAddress` | `types=address&permanent=true`. Up to 2 attempts, 1.5 seconds each. An open breaker is not retried |
| Ride public-place classification | `GetReverseSearchSuggestions` | Search Box `reverse`, `limit=5`, `types=poi,address`. Cached as `reverse:{communityID}:{lat4}:{lng4}` for 10 minutes |
| City and state | `ReverseGeocodeCity` | Geocoding v6 `reverse` with `types=place` |

`resolveRideLocation` in `internal/service/ride/ride_locations.go` rejects a stop when the permanent geocode doesn't return an `address` type. The error types are `ErrorTypePickupAddressRequired` ("Confirm your pickup") and `ErrorTypeDestinationAddressRequired`. A 401 or 403 from Mapbox on a `permanent=true` request becomes a server error, because it means the account isn't licensed for permanent geocoding.

`populatePickupDetails` and `selectDestinationFeature` in `rider.go` mark a stop public when a `poi` feature with categories exists that doesn't map to `misc`, `airport`, `bus_stop`, `subway`, or `train_station` (`nonPublicCategories`).
...
<AccordionGroup>
  <Accordion title="The app starts a new session on every keystroke">
    `useSearchSuggestions` never sends `session_token`, so the server mints a new UUID for each suggestion request. Only the last request's token reaches the retrieve call. Provider session pricing then sees many one-request sessions instead of one per search, and the `search_session` hash only holds the last response.
  </Accordion>
  <Accordion title="The app never sends search_purpose">
    Every mobile search is `general`. The pickup-only hard restriction to the community box never applies from the app, so a pickup search can return places outside the service area. The ride request then fails with **Pickup Outside Service Area**.
  </Accordion>
  ...
  <Accordion title="Likes never age out of top-location ranking">
    Selections are limited to 30 days, but likes count forever and follow the liker's current community. A place liked long ago by users who moved still earns 10 points each. The operator guide describes both signals as 30-day activity.
  </Accordion>
  <Accordion title="Communities without a settings row show top locations">
    `ListTopLocations` treats a missing settings row as enabled for every non-default community. The dashboard describes top locations as hidden by default, which only holds once a settings row exists.
  </Accordion>
