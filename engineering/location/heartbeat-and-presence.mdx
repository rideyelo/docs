...
Admins see this data on the dashboard **Live** page. For the driver background task that also writes presence, see [Driver location tracking](/engineering/location/driver-location-tracking).
...
<Note>
  `driver:loc:{userID}` is written on every heartbeat, including riders'. The name reflects its only consumers (activation, the vehicles map layer fallback, and the warm-pool markers on the **Live** map), not who writes it. It is not refreshed by `POST /user/location`, so a backgrounded driver whose JS heartbeat timer has stopped keeps a `driver:loc` that ages up to 2 hours.
</Note>
...
| Community online count | `community:online:{c}` score | 120 seconds | Heartbeat response `online_users_count`. The **Live** market cards subtract online drivers from it to count online riders |
| Community driver count | `community:drivers:{c}` score | 120 seconds | Heartbeat response `online_drivers_count`. The **Live** market cards count only members within 10 km of the community box (`populateLivePresence`) |
...
| Warm pool driver | `community:warm:{c}` score | 60 minutes (`warmPoolMaxAge`) | Activation tiers, **Recently active** status in the **Live** driver table |
...
| Dashboard **Live** page | `GetOnlineCount`, `GetDriverMembers`, `GetPresenceFields` | Market cards in `internal/service/dashboard/supply_demand.go` (`populateLivePresence`). Map markers from `/dashboard/presence/map` in `presence.go`. The page polls every 10 seconds |
...
<AccordionGroup>
  <Accordion title="The warm pool is never filled">
    Only `PresenceCache.GoOffline` and `ActivationCache.AddToWarmPool` add members to `community:warm:{c}`. `AddToWarmPool` has no caller outside tests. `GoOffline` is called only by `clearFleetEditPresence` in `internal/service/dashboard/fleet_transfer.go`, when an admin transfers a driver to another fleet or edits their fleet memberships. Going offline in the app never adds a driver. Heartbeats only remove members. The warm pool therefore holds only drivers whose fleet an admin just changed, so activation's warm-pool tier and the **Recently active** status on the **Live** page rarely have anyone in them.
  </Accordion>
  <Accordion title="An online driver off the drive screen drops out of the driver set">
    `community:drivers:{c}` only gets a member when the hash's `screen` is exactly `drive`. An online driver who opens the map tab or profile sends `screen: map` or `profile`, which overwrites the hash. Within 120 seconds they drop out of the online driver count on the **Live** page, disappear from the vehicles layer, and stop being offered as a driver option. They stay `is_online_driver` in Postgres.
  </Accordion>
  <Accordion title="Substring matching catches unrelated routes">
    `pathname.includes('/drive')` also matches `/driver-earnings`, `/driver-marketing`, and `/driver-signup`. Those routes get the 10-second cadence. `/driver-earnings` and `/driver-marketing` map to screen `drive` (or `drive_offline`), so an online driver on those screens still counts as a driver. `/ride` also matches `/ride-matching`.
  </Accordion>
