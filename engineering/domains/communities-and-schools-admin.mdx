...
- **`primary_fleet_id` can't be cleared.** `CommunityUpdate` documents "null to clear", but `UpdateCommunityFields` uses `COALESCE`, and a JSON `null` decodes to a nil pointer. Neither path clears the column.
