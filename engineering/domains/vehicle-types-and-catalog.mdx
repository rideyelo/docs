...
- **`PATCH /dashboard/vehicle-types/{id}` is a full replace.** `UpdateVehicleType` writes every listed column from the body. An omitted `min_seats` becomes 0, which disables the seat floor. An omitted `image_asset` clears the artwork. `Update` also skips `validateConfig`, so it can blank `display_name` and `icon_name`.
- **Some fields can't be changed after creation.** `slug`, `is_active`, and `metadata` aren't in the update SQL. No endpoint reactivates a deactivated type. You need a manual database change.
- **New types are inactive unless you say otherwise.** `Create` inserts `is_active` from the body, and Go's zero value is `false`. A type created without `"is_active": true` never enters the registry.
