...
`workspace-quality:ci` is an Nx target that depends on six cached checks. Every one depends on `api-contract` (`make api-check`), so a stale generated client fails first.
...
`make check` runs the same six steps as `ci.yml`, in one job:
...
Mobile E2E uses [agent-device](https://agent-device.dev/) replay files, not Maestro. `yelo-frontend/E2E.md` is the operating guide, and `yelo-api/docs/E2E_CONTROL_PLANE.md` documents the fixture API.
