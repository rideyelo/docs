...
Build-time values come from GitHub repository variables (`VITE_API_URL`, `VITE_CLERK_PUBLISHABLE_KEY`, `VITE_SENTRY_DSN`, and the rest). They are inlined into the bundle, so never put a secret in a `VITE_` variable.
...
<Tabs>
  ...
  <Tab title="Release branches">
    ...
    - `sync-frontend-release.yml` in `yelo-api` runs hourly and keeps a rolling draft pull request that squash-merges `main` onto the release branch named by the `PREVIOUS_RELEASE_BRANCH` variable. `app.json` and `eas.json` stay release-owned. Review it for OTA safety before you mark it ready.
