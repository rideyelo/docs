...
Lowest `finalScore` wins. With exactly one candidate, both ranges are 0, so it wins with a score of 0. Its raw `queueLoad` is still computed and recorded on the `ride_auto_queued` event.
...
2. Calls the `assignRide` callback, wired in `internal/service/service.go` to `RideService.AutoAssignRide`. It passes an `AutoQueueDecision` with the ride's `confirm_time`, the number of scored candidates, and the winner's deadhead minutes, final score, and queue load.
...
- Records `ride_auto_queued` with `driver_id` plus the decision fields (`elapsed_since_confirm_ms`, `candidate_count`, `deadhead_minutes`, `final_score`, `queue_load`), refreshes the queue summary, and calls `RecoverIdleQueue`. If the driver has no active ride, this routes from the driver's GPS, authorizes payment, and promotes the queue head right away. See [Queue promotion and recovery](/engineering/matching/queue-promotion).
...
- **A card failure during immediate promotion requeues the ride.** `RecoverIdleQueue` runs as the assigned driver, so a failed clone or authorization calls `Cancel` as that driver and takes the requeue path, appending them to `declined_drivers`. `AutoAssignRide` then sees the ride is no longer theirs and returns the error, so `assignWinner` logs "may have been claimed" and nobody is pushed. The requeued ride has a fresh `confirm_time` and no attempted flag, so a later pass can pick another driver.
