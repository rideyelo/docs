...
| 6 | `eta_unavailable` | The rough ETA filter dropped the driver, or the active-ride or queue-summary lookup failed |
...
1. `enrichWithQueueSummaries` checks for an active ride and loads the cached queue summary. A `NotFound` from `GetDriverActiveRideInfo` means the driver is idle and they stay a candidate. Any other lookup error drops them. If the driver has an active ride, it counts that ride and adds its remaining time, rounded up to whole minutes. The origin becomes the active ride's dropoff when the queue is empty.
