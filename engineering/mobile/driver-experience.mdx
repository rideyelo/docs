...
<Warning>
The accept button auto-accepts. `RideAcceptButton` fills over 30 seconds (`durationSeconds={30}` in `RideRequestCard.tsx:365`) and calls `onAutoAccept` when it completes. The label reads "Accepting in [N]s". The only way to avoid it is to tap **Decline**.
</Warning>
...
| Error | `EarningsUnavailable` with retry |
...
- **Cash out** appears only when `instant_eligible`. **Cash out $X now** / "Arrives in seconds" opens `CashOutSheet`: available balance, **1% instant fee**, **You'll receive**, and "Skip the fee by waiting for your free weekly payout on [date]." It posts `POST /earnings/payouts/instant` with `amount` equal to the balance minus the 1% fee.
...
- **Silence means accept.** A driver who looks away from a ride card for 30 seconds accepts it. Pausing only happens while the decline sheet is open.
...
- **Earnings can spin forever.** `driver-earnings/1-hub` shows a spinner whenever `payout_source` is neither `fleet` nor `personal_stripe`, for example before any payout setup.
- **Cash-out fee is computed on the client.** `CashOutSheet.tsx:14` hard-codes 1% and ignores `GET /earnings/payouts/instant-quote`. A failed payout is swallowed with the comment "Mock can't fail" (line 52).
- **Waze and Apple Maps fall back to Google.** See [Permissions](/engineering/mobile/permissions-and-device#gotchas-and-edge-cases). On Android, choosing Apple Maps opens `maps.apple.com` in a browser.
- **`StatusGuard` matches by substring.** `pathname.includes('/drive')` also matches `/driver-earnings`, `/driver-signup`, and `/driver-marketing`, so an online driver can sit on those screens without being pulled back to the map.
- **License step is skippable in dev.** `5-license.tsx:76` lets `__DEV__` builds continue without photos.
