...
A banned account isn't signed out. Other calls return 401 `AccountBannedError`, which nothing handles, and `/authv2/status` keeps succeeding because it doesn't check bans.
...
3. If the user closes the browser, the entry screen shows **Having trouble? Continue in app**, which starts the native flow on a fresh attempt. After two cancellations, the next **Get Started** tap goes native automatically, and later taps stay native for the life of the provider.
...
A callback that arrives when no browser session owns it (for example, after the app was killed) is routed by `app/+native-intent.tsx` to `/auth-return`. The app then resumes the hosted transaction with the persisted secret and PKCE verifier (`resumeHostedTransaction`).
...
<Note>
Reserved test numbers use area code 555 (`+1555` plus seven digits) and accept OTP `12345` in every environment. This is intentional. E2E actors use the same reserved range. See [Test, E2E, and internal auth paths](/engineering/identity/test-and-internal-auth) and [Testing](/engineering/testing).
</Note>
