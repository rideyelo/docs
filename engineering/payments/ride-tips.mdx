...
| Rider taps rate twice with the same tip over the buffer | Second call cancels the first tip intent, then reuses the idempotency key. Stripe replays the first response, so the rider gets the canceled intent back |
