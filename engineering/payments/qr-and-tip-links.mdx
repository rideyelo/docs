...
`QrPaymentService.CreateConfig` creates these on the fleet's connected account:
...
`UpdatePricing` creates a new price and payment link, updates the Dub destination (a Dub failure aborts the update), deactivates the old payment link (failure only logged), then saves. `DeactivateConfig` deactivates the payment link and sets `is_active = false`.
...
- **Deleted accounts.** If a fleet's account is replaced with **Unlink**, configs keep their old Stripe product and payment link IDs. New checkouts use the new account, but `UpdatePricing` tries to reuse the old product ID on the new account and fails.
