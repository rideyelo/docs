...
| Status | `GET .../status` | `GetFleetStripeStatus` | Live read: `payouts_enabled`, `charges_enabled`, requirements, `disabled_reason`, and a login link when details are submitted. A login link failure is logged and returns an empty `dashboard_url` instead of failing the request |
