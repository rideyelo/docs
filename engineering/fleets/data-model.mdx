...
| `name` | none | Display name. Two code paths compare it literally to `'Yelo'`: `assignFleetBadges` in `service/ride/rider.go` (verified-student badge) and `ListRideT0NotificationTargets` in `queries/community.sql` (founding-driver filter). Renaming the Yelo fleet changes both behaviors. |
...
| `stripe_merchant_id` | `NULL` | Connected account ID. Indexed (`idx_fleets_stripe_merchant_id`) for webhook lookups. Several fleets can share one ID (fleet cloning). Cleared to `NULL` on soft delete. |
...
| `is_student_only` | `FALSE` | `filterRideTypesForRider` hides the fleet's ride types from non-student riders. Migration `000231` set it for fleets named `Yelo`. |
| `is_active` | `TRUE` | Part of payout readiness and the `ActiveFleetCommunityServices` view. Set `FALSE` by fleet-merchant creation (until Stripe completes), unlink, and delete. Set `TRUE` only by `ActivateDashboardFleet` from the Stripe webhook. |
| `deleted_at` | `NULL` | Soft delete marker. Most reads filter on it. |
...
| `driver_join_code_rotated_at` | `NULL` | Set on rotate and on disable. |
...
Membership rows. Primary key `(fleet_id, driver_id)`. Both foreign keys `ON DELETE CASCADE`, but fleets are only soft-deleted, so memberships survive deletion.
...
There is no `Users.active_fleet_id` column. The API derives `User.ActiveFleetId` in `GetUserProfileByID` (`queries/user_profile.sql`) with a lateral join on `FleetDrivers WHERE is_active ORDER BY fleet_id LIMIT 1`. That join doesn't check whether the fleet is deleted or inactive.
...
| `updated_by` | Clerk user ID, or a system actor such as `pricing_configuration`, `community_archive`, or `primary_fleet_configuration`. Copied into history. |
...
Created in `000168`. `email` is globally `UNIQUE`, so an email can be whitelisted for only one fleet. `UpsertFleetDriverWhitelist` moves an existing email to the new fleet and reactivates it. `added_by` became nullable in `000170`, and the dashboard handler always passes `nil`.
...
| Concern | Path |
| --- | --- |
| Schema | `yelo-api/internal/data/sqlc/schema.sql`, `yelo-api/migrations/000157_*` through `000344_*` |
...
- **Soft delete leaves memberships and prices.** `DeactivateDashboardFleet` only sets `is_active = FALSE`, `deleted_at`, and `stripe_merchant_id = NULL`. `FleetDrivers`, `DriverApplications.payout_fleet_id`, open `RidePriceParameters`, `FleetVehicles`, and the join code all remain. See [Stripe and lifecycle](/engineering/fleets/stripe-and-lifecycle#delete).
- **`ActiveFleetId` can point at a deleted fleet.** The profile query doesn't join `Fleets`. `reconcileFleetAssignmentForUser` returns early whenever `ActiveFleetId` is set, so it never repairs this state.
- **The active-fleet index is enforced per row.** `SetDriverActiveFleet` flips every membership in one `UPDATE` (`SET is_active = (fleet_id = $1)`). A non-deferrable unique index is checked as each row changes, so whether this statement conflicts depends on the order in which PostgreSQL visits the rows. The join path avoids the problem by deactivating other rows in a separate statement first.
