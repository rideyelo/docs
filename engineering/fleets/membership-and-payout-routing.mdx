...
| Admin transfer, legacy add by email | `TransferDriver` via `DashboardService.TransferUserFleet` | Yes | Yes | No | Takes the driver offline; an unfinished ride blocks | Yes, under row lock |
| Admin membership edit | `UpdateDriverMemberships` | Yes, when the active fleet changes | Yes, or clears it for an empty set | No | Takes the driver offline; an unfinished ride blocks | New and selected fleets, under row lock |
| Driver switch | `sqlUserRepository.SetDriverActiveFleet` | Yes, same statement | Yes | Yes | Yes | Target must be active and not archived; payout-ready for fleet-merchant |
| Automatic enrollment | `AddDriverToDefaultFleet` from `reconcileFleetAssignmentForUser` | Not applicable (no memberships) | No | No | No | Default fleet must be joinable and owner-operator |

Every path except automatic enrollment keeps the active fleet and payout fleet aligned. Only redeem, admin assign, and the driver switch move `Users.community_id`. Automatic enrollment only writes `FleetDrivers`, and only for a driver with no non-archived memberships.
...
<Steps>
  <Step title="Lock the driver">
    `lockRideParticipants` takes the shared assignment lock. `LockDriverForFleetSwitch` then selects the user `FOR UPDATE` and computes `has_ride` (any ride not in `complete` or `pending_rider_rating`) and `has_session` (an open `DriverSessions` row). If the driver is online or either flag is set, the switch fails with `ErrorTypeFleetJoinBusy` (HTTP 409, **Finish Driving First**).
  </Step>
  <Step title="Load the target">
    `GetFleetSwitchTarget` joins `Fleets` to `FleetDrivers` and requires the fleet to be active and not archived, locking the fleet row `FOR UPDATE`. A missing membership or an unavailable fleet returns **driver is not a member of this fleet** (404). The query computes `payout_ready`.
  </Step>
...
flowchart TD
  ...
  B -->|No| F["Default community has primary_fleet_id?"]
  F -->|Yes| G["AddDriverToDefaultFleet(primary_fleet_id)"]
  F -->|No| H["Stay without a fleet"]
```

Only the default community (`Communities.is_default`, via `Community.GetDefault`) supplies automatic enrollment. The driver's own community primary fleet and the email whitelist are never used.

`AddDriverToDefaultFleet` runs in one transaction. It re-reads `ListDriverFleets` and does nothing if the driver gained a membership concurrently. It then locks the fleet with `LockFleetForJoin` and fails with **default driver fleet must be an active personal-payout fleet** unless the fleet is joinable and uses `owner_operator`. It doesn't set `payout_fleet_id` or move the driver's community.

Tie-breaks and failure modes:

- `ListDriverFleets` has no `ORDER BY`, so "first fleet" is whatever order PostgreSQL returns. It excludes archived fleets but not inactive ones.
- If `fleets[0]` is inactive, or a fleet-merchant fleet that isn't payout-ready, `SetDriverActiveFleet` returns an error and the function doesn't try the next membership.
...
<Note>
  Because only one membership can be active, a fleet-managed driver's payout fleet is effectively required to be their active fleet. Every current writer that changes the active fleet also rewrites the payout fleet, so the two only diverge for rows written before these paths existed.
</Note>
...
| Default enrollment transaction | `addDriverToFleet`, `defaultFleetAssignmentNeeded` in `yelo-api/internal/data/repository/user_fleet.go` |
...
- **Payout fleet and routing fleet can differ.** A driver with `payout_fleet_id = NULL` goes online on personal Stripe readiness. If their session fleet has custom payouts, fares still go to the fleet's account.
- **Capture recomputes the account.** `Complete` calls `selectStripeMerchantID` again instead of reusing `Rides.stripe_merchant_id`. If an admin unlinks Stripe or toggles `has_custom_payouts` while a ride is in progress, the capture targets a different connected account than the one that holds the authorization. Archive can't cause this, because open rides and sessions block it.
...
- **Busy means any unfinished ride.** `has_ride` counts every ride not in `complete` or `pending_rider_rating`, including `queued` rides, so a driver with a queued ride can't switch fleets, and an admin can't transfer or edit their memberships.
- **Admin edits skip the auth cache.** `clearFleetEditPresence` invalidates the profile cache and presence, but not the auth cache, unlike the join and switch paths.
- **Default enrollment skips caches.** `AddDriverToDefaultFleet` invalidates nothing. `RefreshOnlineStatus` reloads the user from the database right after, so going online sees the new fleet.
