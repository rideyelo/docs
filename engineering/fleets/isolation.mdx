...
`assignFleetBadges` gives options whose `FleetName == "Yelo"` the verified-student badge and every other option the fleet badge. Each option keeps at most two badges, ranked verified-student, fleet, cheapest, fastest.
...
- The vehicle matches the ride's vehicle type, is approved, has enough seats, isn't owned by another fleet, and is available to the driver for this fleet.
...
`is_student_only` is enforced on the rider side only. `filterRideTypesForRider` removes the fleet from ride types for non-student riders, and `determineFallbackRidePrice` applies the same filter before choosing a fallback fleet. Nothing on the driver side reads it.
...
- **Fleet identity is by name in two places.** Badges and T0 compare `Fleets.name` to `'Yelo'`.
