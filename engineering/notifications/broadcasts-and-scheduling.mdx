...
The API offers four ways to reach users: the **Send Notification** sheet, scheduled sends from the same sheet, market notifications, and single-user pings. All four become push notifications through the standard pipeline. None of them send SMS.

<Note>
Only the **Send Notification** sheet is reachable in the dashboard today. The **Supply & Demand** page was replaced by **Live**, which has no notify or ping controls. `NotifyMarketDialog`, `PingDialog`, and `DriverOpsPage` still exist in `yelo-web`, but no route renders them. The market notify and ping routes below are API-only.
</Note>
...
| Notify market | `POST /dashboard/live/markets/{id}/notify` | Admin only: `dashboardScopeAllows` rejects interns and fleet managers on `/dashboard/live`. Community resolved from the path | `community_broadcast` | Community scope |
| Ping driver | `POST /dashboard/live/drivers/{id}/ping` | Admin scope plus `requireAdmin` | `dashboard_ping_driver` | One user |
| Ping rider | `POST /dashboard/live/riders/{id}/ping` | Admin scope plus `requireAdmin` | `dashboard_ping_rider` | One user |
...
| Scheduled rows SQL | `yelo-api/internal/data/repository/notification.go`, `queries/notification_log.sql` |
...
| Dashboard UI | `yelo-web/src/sites/dashboard/pages/NotificationsPage.tsx`, `src/components/notifications/SendNotificationSheet.tsx`. Unrouted market and ping dialogs: `src/components/live/NotifyMarketDialog.tsx`, `PingDialog.tsx` |
