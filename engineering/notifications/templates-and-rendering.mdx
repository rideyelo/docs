...
<Warning>
The variant insert matches on `label`. If an admin renames the seeded `default` variant, the next boot inserts a fresh `default` variant with weight 100 and `is_active = true`. The active total then exceeds 100, `orchestrator.Allocate` fails, and every emit of that code is canceled until someone fixes the weights. Keep one variant labeled `default`, or deactivate it rather than renaming it.
</Warning>
