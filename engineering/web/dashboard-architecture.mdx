...
| --- | --- | --- |
| `/` | `RootRoute` (redirect only) | `admin` → `/live`, `intern` → `/community`, `fleet` → `/fleets` |
| `/kpi` | `KpiPage` | `admin` |
...
<Note>
`src/sites/dashboard/pages/` still contains modules with no route of their own, such as `OverviewPage`, `DriverOpsPage`, `SettingsPage`, and `ZonesPage`. `CancellationsPage` and `UncompletesPage` render inside `/rides`, and `SearchesPage` renders both there and at `/searches`. `SchoolsPage`, `VehiclesPage`, and `ActivationPage` remain in `routeImporters` so the search index can read their copy, but their routes are redirects. `routeImporters` still maps `/` to `OverviewPage`, and the search index gives `/` the roles `admin` and `intern`, so global search can return Overview sections that land on `/live` or `/community`. `/supply-demand` has no route and renders **Coming Soon**. Check `App.tsx` before you assume a page file is reachable.
</Note>
...
`ProtectedRoute` in `App.tsx` takes an `allowedRoles` array. While roles load, it renders the skeleton `PageLoader`. If the role is not allowed, it redirects `fleet` users to `/fleets` and everyone else to `/`. `RootRoute` redirects `admin` to `/live`, `intern` to `/community`, and `fleet` to `/fleets`, and shows an "Unauthorized — You do not have a valid role assigned" message for any other role.

### Sidebar

`src/components/Sidebar.tsx` hardcodes `navGroups`. The sidebar header holds the brand link, which opens the role's home page, and Clerk's `UserButton` account menu. `SidebarCommunityScope` sits below it; see [Community scope](#community-scope).
...
`SidebarCommunityScope` (`src/components/SidebarCommunityScope.tsx`) renders the scope control at the top of the sidebar. It loads all communities with `useCommunityGetAllCommunities` and filters them by role:

- **Admins** get `CommunityDropdown`. On `/pricing`, `/communities`, `/rides`, and `/drivers`, the page reads its market from `?community=`, and choosing a community writes it there and clears `scope`. Without `?community=`, `/pricing` and `/drivers` fall back to the remembered community, while `/communities` and `/rides` (unless `?scope=community`) show **All markets**. `/kpi` shows a static **All markets** label.
- **Interns** see a read-only **Assigned community** card.
- **Fleet managers** see a read-only card with the current fleet and community. There is no dropdown, so a fleet manager with assignments in several communities can only switch through a community result in global search.

The `Topbar` holds only the sidebar toggle, `GlobalSearch`, and `RecentViews`. It also loads the community list, filtered by role, for search results.
...
| Maps | `mapbox-gl` with `VITE_MAPBOX_API_KEY`; styles follow the theme through `mapboxStyleForTheme`; `@mapbox/mapbox-gl-draw` for zone editing; dark-mode popup styles in `src/sites/dashboard/styles.css` |
