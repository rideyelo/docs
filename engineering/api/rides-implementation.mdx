...
flowchart LR
  ...
  F -->|complete| G[pending_rider_rating]
  G -->|rider rates, or tip window expiry| H[complete]
```
...
- **Complete** (`Complete`) selects the Stripe merchant (fleet or driver) and always moves the ride to `pending_rider_rating`. With tips enabled and a nonzero price, it sets a 24-hour `tip_window_expires_at` and defers capture. Otherwise it captures the fare immediately, and the ride stays in `pending_rider_rating` until the rider rates it.
...
<Tabs>
  <Tab title="Redis (default)">
    ...
    `AddRide` writes all three in one `TxPipeline` and falls back to the in-memory board if it fails. `RemoveRide` falls back only when the `SREM` fails. Nothing reads the set or the refresh key; they only feed `GET /ride/board/{id}` through `ride:details`. Drivers read the board from Postgres. See [Ride board](/engineering/matching/ride-board).
  </Tab>
  <Tab title="In-memory">
...
`rideevents.EventRecorder` writes the ride timeline to the `"RideEvents"` table asynchronously. See [Ride events](/reference/ride-events).
...
- `main.go` calls `Stop` during shutdown. `Stop` closes the channel and returns without waiting for it to drain, and the insert goroutine uses the already-canceled signal context. Events still buffered at shutdown can be lost.
...
<Steps>
  ...
  <Step title="Notify drivers by tier">
    For each ride, the service loads the community's activation config (cached 5 minutes), picks the lowest due tier that hasn't fired yet, and notifies offline drivers near the pickup. The warm-pool pass reads a Redis sorted set that nothing writes, so candidates come from `ListNearbyInactiveDrivers`. Notified drivers are tracked in Redis so earlier tiers don't repeat them, and a per-driver cooldown blocks re-paging across rides. See [Activation tiers](/engineering/matching/activation-tiers).
  </Step>
...
Both are min-max normalized. The final score is 0.5 × efficiency + 0.5 × equity, and the lowest score wins. Scores within 0.05 of each other tie-break on the smaller queue. When the eligibility query returns nobody, or filtering and Mapbox leave no scored candidate, the ride is flagged in Redis (`ride:aq_attempted:{rideId}`) for 1 hour so the pass skips it. When there are no candidates at all, the fleet is also notified about the missed ride. Read errors, a missing session, and a lost assignment race don't set the flag, so the ride is retried on the next tick.

The winner is assigned through the callback that `NewService` passes to the activation service, which calls `RideService.AutoAssignRide` with the scoring decision. That method runs `Ride.AutoQueueAssign` (status `queued`), records `ride_auto_queued` with the candidate count, deadhead, score, and queue load, refreshes the queue summary, and calls `RecoverIdleQueue`. If the driver has no active ride, the queue head is promoted to `driver_en_route` through `prepareAndPromoteQueuedRide`, which authorizes payment and recalculates the pickup route before `PromoteQueuedRide`.
