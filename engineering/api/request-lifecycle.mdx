...
- In `dev`, CORS is disabled and any `Origin` is echoed back.
...
The router registers these in order, so they wrap every Fuego route:
...
| 4 | `otelInstrumentation` | Creates the span (named `METHOD /route/pattern`), records request and response size, inflight gauge, slow requests, and error counters |
...
| `/metrics`, `/debug/pprof/` | `metricsAuthMiddleware` | Bearer `METRICS_AUTH_TOKEN` when set |
...
| `/debug/pprof/` | pprof, when `PPROF_ENABLED` is true |
