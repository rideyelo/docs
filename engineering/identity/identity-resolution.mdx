...
- **Unbanning doesn't restore sign-in.** A banned user who tries to sign in gets a permanent `banned_user` tombstone. Flipping `is_banned` back to false doesn't remove it. Delete the `AuthDeniedSubjects` row too, keyed by the HMAC, which you can only compute with the subject key.
...
- **Phone owners include unverified rows, candidates don't.** Phone recovery counts any non-deleted `Users` row with the number. First-time candidate matching counts only `is_verified` rows. An unverified legacy row can therefore block an OAuth sign-up with `account_merge_required` at the phone step.
- **Web sign-ins can collide on email.** The web channel never creates provisional shells. A web Google sign-in whose email matches an unverified legacy row fails with `verify_existing_account`.
