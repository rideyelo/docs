...
<Warning>
Because accept sets `accepted_at`, an attempt can't be rotated after the user taps a method on the hosted page. An expired attempt can't be rotated either. Any retry after that point must use a new attempt ID. Mobile already starts a fresh attempt ID when its draft is marked accepted or the attempt is older than 15 minutes, and retries with a fresh ID once on a 409.
</Warning>
...
<Warning>
Known bug: `AuthHandoffCodes.transaction_id` is unique, and `CreateAuthV3HandoffCode` has no `ON CONFLICT` clause. Nothing deletes the existing code first. A transaction stays `verified` after resolution creates its handoff, so the `handoff` branch of resume hits a unique violation (`23505`), which isn't retryable and returns a 500 instead of a fresh code. `resume_test.go` doesn't catch it, because it sets `verified` without a prior handoff and the fake repository doesn't enforce uniqueness. Check for this when you debug a "lost callback" report.
</Warning>
...
- **Rotating the first state HMAC key.** Derived nonces change. A pending attempt that retries authorize with the same attempt ID still succeeds, because the upsert overwrites `nonce_hash`, `state_hash`, and `code_challenge`, and the client receives the new nonce. Existing secrets still verify with the old key.
- **Mobile follows the server expiry.** The mobile draft uses the server's `expires_at` (10 minutes by default). `AUTH_V3_TRANSACTION_TTL_MS` (15 minutes) is only a fallback when `expires_at` can't be parsed, and `AUTH_V3_ATTEMPT_TTL_MS` (15 minutes) only decides whether the local attempt ID is reused. Mobile reuses a draft only while it has more than 60 seconds left and the same `native_phone_step` value.
...
- **Accepting twice is safe, changing versions isn't.** Re-accepting with the same versions is idempotent. Request versions that differ from the current config fail validation with 400. A 401 `auth transaction could not be accepted` only happens when the config versions changed after the first accept, because `AcceptAuthV3Transaction` refuses to rewrite existing acceptance.
