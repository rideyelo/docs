...
- **Metadata updates merge, they don't replace.** `UpdateAccessUser` only replaces `fleet_assignments` when the new role is `fleet`, so changing a fleet manager to an intern leaves the old assignments in their metadata. `populateClerkUserContext` prefers `fleet_assignments` and overwrites both `FleetID` and `CommunityID` from the first one. The API then scopes the new intern to the stale fleet's community, while the web app uses `publicMetadata.community_id`. The sidebar shows one community and the API allows another. Clear `fleet_assignments` in Clerk when you change a fleet manager's role.
...
- **Unknown roles see "Unauthorized".** A Clerk user with no role can sign in to the dashboard. `RootRoute` shows "You do not have a valid role assigned", and every API call returns 401 from the scope gate. Valid roles land on `/live` (admin), `/community` (intern), or `/fleets` (fleet).
- **The invite toast doesn't distinguish updates.** `InviteUserSheet` always shows "Invitation sent to ..." even when the API returns status `updated` for an existing user.
