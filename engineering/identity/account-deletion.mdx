...
`UserHandler.Delete` in `internal/server/http/user/user_handler.go` ignores the `{id}` path parameter. It always deletes the authenticated caller. No admin or dashboard deletion endpoint exists.
...
<Steps>
  ...
  <Step title="Delete the profile photo">
    If `photo_url` is set, delete `profile_pictures/<user id>/<last path segment of photo_url>` from storage.
  </Step>
...
| Data | Location | Notes |
| --- | --- | --- |
| The `Users` row | `Users` | ID, community, school, account type, referral code, Stripe IDs, `is_test`, timestamps |
...
- **Retries can get stuck on Stripe.** Stripe deletion runs first and `stripe_customer_id` is never cleared. If a later step fails, a retry calls Stripe again for an already-deleted customer. Unless Stripe accepts that, the retry fails at step 2 every time.
- **Native Apple grants are revoked with the web client ID.** `ExchangeNativeCode` obtains the refresh token as `AUTH_V3_APPLE_NATIVE_CLIENT_ID` (the bundle ID), but `RevokeRefreshToken` always sends `AUTH_V3_APPLE_SERVICES_ID`. When the two differ, Apple may reject the revocation, which blocks deletion with 503 for every user who signed in natively with an authorization code. Verify this against Apple before relying on it.
...
- **Proofs block re-verification.** A retained `EducationVerifications` row keeps its unique provider subject. The same school Google account or StudID identity can't verify a new account later.
