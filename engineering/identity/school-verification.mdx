...
| Signed-in user | Yes | No collision check, no student change, and nothing written. The call still returns success. |
...
- **The mobile disconnect copy is wrong for the last proof.** The **Accounts** screen says disconnecting "does not remove your existing student status". The API downgrades the account to `community` when the last proof is removed.
...
- **Proofs outlive deleted accounts.** Account deletion doesn't delete `EducationVerifications`. Because provider subjects are unique per provider, the same school Google account or StudID identity can't verify a new Yelo account later. It fails with `identity_conflict`.
