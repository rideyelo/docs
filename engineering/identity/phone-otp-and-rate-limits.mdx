...
Session refresh, logout, the onboarding routes, and the authenticated `/authv3/user` routes other than link authorize have no durable Auth v3 limit. They still pass through the API-wide in-memory per-IP token bucket (50 requests per second, burst 100, per instance) that applies to every route. `POST /user/link/authorize` shares the `authorize` bucket with `POST /authorize`.
...
- **Test numbers still consume counters.** Reserved `+1555` sends skip Twilio by design, but they go through every reservation.
- **Fallback sends are never retried.** This is deliberate (`phone_fallback.go` says so). A failed toll-free send leaves the row claimed, so the user never gets a fallback. They must tap resend.
