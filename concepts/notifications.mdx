...
| SMS | Twilio, or Photon for friend bumps when configured | Friend bumps, and phone sign-in codes through Twilio Verify. |
...
<Warning>
  A second broadcast to the same community in the same 5-minute window is dropped, even with a different recipient group or copy. The broadcast event key is the community plus a 5-minute time bucket, and the API still reports success.
</Warning>
...
### Market notifications and pings

The API can also notify a whole market or ping one driver or rider. These emit `community_broadcast`, `dashboard_ping_driver`, and `dashboard_ping_rider`. The dashboard has no controls for them today, because the **Supply & Demand** page that hosted them was replaced by **Live**. See [Operator broadcasts](/engineering/notifications/broadcasts-and-scheduling).
...
| Driver application | On the first upload of **Profile Photo**, **License Photos**, and **Vehicle Created**. **Stripe Signup** posts on every Stripe `account.updated` event for a complete account, so it can repeat. Includes a **View application** link. | Driver updates |
