...
| `driver_en_route`, `driver_waiting`, `in_progress` → `complete` | Admin | `POST /dashboard/rides/{id}/silent-cancel` (free rides only, no notifications) | `DashboardService.SilentCancelRide` → `SilentCancel` (`internal/data/repository/ride_admin_cancel.go`). See [Admin silent cancellation](/concepts/cancellations#admin-silent-cancellation) |
...
<Warning>
`UnqueueDriverRides` also sets `fleet_id` to null and keeps the original `confirm_time`. Drivers only see board rides for their session fleet, so an unqueued ride is invisible to every driver, and it expires on the next sweep if it was confirmed more than 10 minutes ago. The rider gets no push when this happens. See [Queue promotion and recovery](/engineering/matching/queue-promotion#offline-unqueue).
</Warning>
