...
At accept, the chosen account is saved as `Rides.stripe_merchant_id`. Both selections are recorded in a `merchant_selected` ride event with `merchant_type` of `fleet` or `driver`. Cancels, tip captures, and deferred fare captures use the saved account. The immediate capture at complete does not: `Complete` recomputes the account from the driver's current session fleet and captures on that, without updating the ride row. See [Ride payment intents](/engineering/payments/ride-payment-intents).
