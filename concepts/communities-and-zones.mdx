...
- **Primary Vehicle Type** and **Primary Fleet**. The primary fleet orders rider options, supplies the fallback ride type, and supplies rider-facing operating hours. Only the default community's primary fleet enrolls drivers automatically. See [Fleets](/concepts/fleets#fleet-drivers-and-independent-drivers).
...
Riders see ride types, not fleets. A ride type is a price row for one fleet and one vehicle type in one community. It reaches riders while the price row is open and the fleet has a live driver or the row is featured. Service coverage doesn't gate riders: retiring a fleet's service community keeps its prices open. Archiving the fleet or the community ends them. See [Fleets](/concepts/fleets#service-communities).
