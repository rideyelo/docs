...
<Warning>
  The app counts `pending` steps as done on the hub, so drivers reach **Submitted** before any approval. A rejected (`failed`) step shows **NOT APPROVED** and blocks the hub, including a rejected profile photo that already has an uploaded file. Going online still requires `complete`.
</Warning>

## Admin review

Admins review applications from the **Drivers** page (`/drivers`) in the dashboard. The review sheet lists each requirement with **Approve** and **Reject** buttons. Rejecting asks for a reason, which the API stores as `rejection_reason`. The mobile app never displays it, so the driver only sees **NOT APPROVED** on that step.
...
<Warning>
  The approval push deep-links to `/driver-signup/1-application-status` (from the seeded `driver_application_approved` template). The app has no such screen, so tapping the push opens the "This screen doesn't exist." page.
</Warning>
...
| Joined through a fleet link | The join tries to activate the driver immediately. An owner-operator fleet (custom payouts disabled) doesn't manage payouts, so its drivers still need personal Stripe. |

On activation, Yelo assigns an active fleet if the driver has none: the primary fleet of the default community. See [Fleets](/concepts/fleets#fleet-drivers-and-independent-drivers).
...
When `DRIVER_VERIFIED_PHONE_REQUIRED` is `true`, drivers must have a verified phone number before they upload photos, register a vehicle, create a Stripe account, activate, or go online. The flag defaults to `false`. Auth v3 accounts created in the app always have a verified phone. Accounts created with Google or Apple on the web signup site can have no phone, and so can legacy accounts.
