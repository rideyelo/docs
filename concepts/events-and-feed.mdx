...
The dashboard only offers interns **Submit for Review**. The API itself doesn't check the role, so an intern can publish or archive an event in their own community by calling the status endpoint directly.
...
The dashboard API lives under `/dashboard/communities/{id}/events`. The old `/spot` and `/feeds` routes redirect to `/events`.
...
- Pages hold 10 items. Each source is limited to 30 rows, but ads are not, so a busy community can have more than 3 pages. The live Spot makes the first page 11 items.
- Friend bumps come before rides, so many live bumps can push every ride off the first page.
