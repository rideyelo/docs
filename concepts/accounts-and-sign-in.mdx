...
| `phone_number` | Only ever stored after a successful OTP check, so a non-empty value means "verified phone". |
...
A successful verification sets `school_id`, `account_type = 'student'`, and `is_verified = true`. StudID proofs are stored in `EducationVerifications`, and users manage them from the profile **Accounts** screen.
...
<Steps>
  ...
  <Step title="Verify a phone number">
    If the user signs in with Apple or Google in the app and the account has no phone yet, Yelo asks for one before finishing. Web-channel sign-ins, such as the dashboard, do not require a phone. See [Provider flows](/engineering/identity/provider-flows#the-supplemental-phone-step). Phone codes are five digits and are sent through Twilio Verify.
  </Step>
...
Sign-in is refused for disabled, deleted, or banned accounts and for ambiguous matches (`AuthV3DeniedReason` in `internal/models/authv3.go`).

When a user deletes their account, Yelo revokes their Apple token, removes their login identities, and scrubs their user row. If Apple revocation fails, nothing is deleted, so the deletion can be retried.
