...
| `is_student_only` | Hides the fleet's ride types from riders without a verified student account, and gives its options the verified-student badge. |
| `has_owner_operators` | Marks the fleet as having owner-operator drivers. |
| `is_active`, `deleted_at` | Inactive or archived fleets can't accept joins and don't serve communities. `deleted_at` marks an archived fleet. |
...
A driver can belong to several fleets but has at most one **active fleet**: the `FleetDrivers` row with `is_active = TRUE`. See [Membership and payout routing](/engineering/fleets/membership-and-payout-routing). The active fleet decides the community they drive in, the rides they can accept, and where their fares go.

When a driver's application activates, or they go online, and they have no active fleet, Yelo picks one (`reconcileFleetAssignmentForUser` in `internal/service/driver/driver.go`):

1. If the driver already belongs to a non-archived fleet, the first one becomes active.
2. Otherwise, the driver joins the primary fleet of the **default** community. That fleet must be an active owner-operator fleet with personal payouts.

The driver's own community primary fleet and the email whitelist are never used. If the default community has no primary fleet, the driver stays without a fleet and can't go online.
...
- The driver isn't a member of the target fleet, or the fleet is inactive or archived.
...
<CardGroup cols={3}>
  ...
  <Card title="Admin membership edit" icon="user-plus">
    An admin adds or removes fleets and picks the active fleet in the user sheet's **Fleet membership** section.
  </Card>
  <Card title="Default enrollment" icon="rotate">
    A driver with no fleet is enrolled in the default community's primary fleet automatically.
  </Card>
</CardGroup>

Join links go through the fleet join service (`internal/service/fleetjoin/fleet_join.go`). A join:
...
Admin membership edits (`PUT /dashboard/users/detail/{id}/fleets`) differ from a join. They keep the driver's community, keep memberships that aren't explicitly removed, take the driver offline, and are blocked while the driver has an unfinished ride. See [Joins and invitations](/engineering/fleets/joins-and-invitations#admin-membership-editing).

The API still stores email whitelist entries (`FleetDriverWhitelist`), but no dashboard page manages them and Yelo doesn't enroll drivers from them.
...
Removing a fleet archives it. Yelo keeps the fleet row, ride and payment history, and the Stripe account reference. Archive is refused until the fleet has no active drivers or payout assignments, no open rides or driver sessions, no community that uses it as **Primary Fleet**, no unarchived vehicles, and no active QR payment links. Archiving disables the join link and whitelist, retires the fleet's service communities, and ends its pricing. See [Archive](/engineering/fleets/stripe-and-lifecycle#archive).
...
| Admin membership edits | `yelo-api/internal/data/repository/driver_fleet_memberships.go`, `internal/service/dashboard/fleet_transfer.go` |
| Fleet archive | `yelo-api/internal/data/repository/fleet_archive.go`, `migrations/000346_fleet_archive_reference_guards.up.sql` |
| Design notes | `yelo-api/docs/owner-operator-fleet-joins.md`, `docs/fleet-owned-vehicles.md`, `docs/fleet-community-dashboard.md`, `docs/admin-fleet-transfers.md` |
