...
A fleet can serve more than one community. Each fleet-to-community pairing is a row in `FleetCommunityServices`. A fleet's prices in a community only reach riders while that service is enabled, the fleet is active, and the community is active and not archived. The `ActiveFleetCommunityServices` view enforces this.
...
A driver can belong to several fleets but has one **active fleet** (`Users.active_fleet_id`). The active fleet decides the community they drive in, the rides they can accept, and where their fares go.
