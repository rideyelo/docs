...
| `cancel_reason` | `text` | Yes | | Reason. The system writes `automatic` for unmatched confirmed rides, `superseded` when a new request replaces an older `requested` ride, and `admin_silent_cancellation` for a dashboard silent cancellation. A driver's requeue reason is also stored here. |
| `cancel_comment` | `text` | Yes | | Free-text comment with the cancellation. A silent cancellation stores the admin's reason here. |
...
- Trigger `guard_fleet_ride_archive` (`BEFORE INSERT OR UPDATE OF fleet_id`) rejects a ride that references an archived fleet. See [Fleet archival](/reference/database/fleets-and-vehicles#fleet-archival).
...
### Admin silent cancellation

`POST /dashboard/rides/{id}/silent-cancel` runs `SilentCancel` in `internal/data/repository/ride_admin_cancel.go`, with queries in `queries/ride_admin_cancel.sql`. It takes the normal ride command locks, requires the caller's expected `lifecycle_revision`, and re-runs `GetRideSilentCancellation`. That query allows only `driver_en_route`, `driver_waiting`, and `in_progress` rides where `COALESCE(discounted_price, price) = 0`, `payment_intent_id` and `tip_payment_intent_id` are null, no `RideOperations` row is `pending` or `quarantined`, the driver has no other `queued` ride, and no `NotificationDeliveries` row for the ride's `Notifications` is `queued` or `sending`.

`SilentlyCancelRide` sets `status = 'complete'`, `cancel_phase` to the prior status, `cancel_user = NULL`, `cancel_reason = 'admin_silent_cancellation'`, `cancel_comment` to the reason, and `complete_time = now()`. `driver_id` is kept. The same transaction advances ride and user revisions and inserts a `ride_canceled` `RideEvents` row directly, with `actor_type = 'system'` and metadata `admin_id` (a Clerk user ID, not a `Users` ID), `reason`, `source`, `notifications_suppressed`, and `cancellation_fee`. No payment, queue, or notification code runs.
...
Writers: `rideevents.EventRecorder` in `internal/service/rideevents/recorder.go` buffers events in a 500-slot channel and inserts them from one goroutine. `InsertMessageTimeline` in `messaging.sql` writes `message_sent` events directly. Admin silent cancellation writes its `ride_canceled` event with `InsertRideEvent` inside the cancellation transaction.
...
- Trigger `guard_fleet_session_archive` rejects a session that references an archived fleet.
...
Writers: `user_driver.sql` and `fleet_vehicle.go`. Readers: ride acceptance, auto-queue promotion, the dashboard **Live** market queries (`queries/supply_demand.sql`), notification recipient selection (`internal/notify/repository/recipients.go`).
