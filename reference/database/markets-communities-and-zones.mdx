...
| `is_default` | `boolean` | no | `false` | Marks the fallback community for users and schools outside every active market. Excluded from coordinate lookup. |
...
| `primary_fleet_id` | `uuid` | yes | | Default fleet. FK to `"Fleets"`. Setting it triggers `ensure_primary_fleet_service`. |
