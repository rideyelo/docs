...
**Indexes.** Unique constraint `Locations_address_key` on `address`, restored by migration `000347_restore_location_address_constraint`. Unique `idx_locations_community_address` on `(community_id, address)`, which the global constraint already implies. Indexes on `community_id`, on `community_id` where `show_on_map`, hash on `mapbox_id` where non-empty, and on `yelo_place_id` where not null.
...
<Warning>
`UpsertCachedLocation` in `queries/places.sql` uses `ON CONFLICT (address)`, which needs `"Locations_address_key" UNIQUE (address)`. Production had that constraint from the original schema without a migration. Migration `000347` adds it when it's missing and skips when a valid, non-deferrable unique constraint on `address` already exists. If duplicate addresses exist, the migration raises `cannot restore Locations_address_key: duplicate addresses require operator review` and the release stops. Its down migration keeps the constraint.

Because `address` is unique across the whole table, the same address can't be stored for two communities, even as curated POIs.
</Warning>
