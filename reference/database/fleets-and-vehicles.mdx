...
| `deleted_at` | `timestamptz` | yes | | Soft delete. Partial index on rows where it is null. |
...
- Assignment on driver onboarding (`reconcileFleetAssignmentForUser` in `internal/service/driver/driver.go`): keep an existing fleet, then use the whitelist entry for the user's email, then the community's `primary_fleet_id`.

## Fleet driver whitelist

`"FleetDriverWhitelist"` pre-assigns a driver to a fleet and community by email before they sign up.
...
- Read by `internal/service/driver/driver.go` for fleet assignment and by `GetWhitelistSchoolMatch` (`queries/education_commerce.sql`) to place a whitelisted user in the right community.
...
| `num_seats` | `integer` | no | `4` | Seats. Dispatch requires `num_seats >= num_riders`. |
